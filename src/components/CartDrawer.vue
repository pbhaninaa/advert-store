  ...
  <Teleport to="body">
    ...
    <Transition name="slide">
      ...
      >
        ...
        <ul v-else class="flex-1 overflow-y-auto px-5 py-4 space-y-3 bg-t-cart-bg">
          <li
            v-for="line in lines"
            :key="line.id"
            class="flex gap-3 border border-t-border p-3 bg-t-surface cart-item"
          >
            <img
              :src="line.image"
              :alt="line.title"
            ...
            <div class="flex-1 min-w-0">
              <h3 class="font-display text-base font-semibold text-t-text truncate">{{ line.title }}</h3>
              <p class="text-t-price font-semibold mt-0.5 text-sm">{{ formatPrice(line.price) }}</p>
              <div class="mt-2 flex items-center gap-2">
                <button
                  type="button"
                  class="h-7 w-7 border border-t-border text-t-text text-sm hover:border-t-text transition-colors rounded-theme-sm"
                  @click="updateQty(line.id, line.qty - 1)"
                >
                  −
                </button>
        ...
        <div v-if="lines.length > 0" class="border-t border-t-border px-5 py-5 bg-t-cart-bg">
          <label class="block text-xs font-semibold uppercase tracking-widest text-t-muted mb-2">
            Size / delivery note
          </label>
          <textarea
            v-model="customerNote"
            rows="3"
            class="w-full border border-t-border bg-t-surface px-3 py-2 text-sm text-t-text focus:border-t-text focus:outline-none theme-input"
            placeholder="e.g. Size M, deliver to Sandton..."
          />
          ...
          <p class="mt-3 text-xs text-t-muted leading-relaxed">
            Opens WhatsApp or email on your phone — you send the order to {{ storeConfig.name }}.
          </p>
...
const { lines, totalPrice, updateQty, removeItem, clearCart } = useCart()
